...
/*   Updated: 2023/07/21 16:16:12 by jschwabe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 	tail = stack->tail;
// 	if (head->prev != tail)
// 		own_error("tail is not head->prev");
// 	if (tail->next != head)
// 		own_error("head is not tail->next");
// 	if (tail->next == tail || head->prev == head || head == tail)
// 		own_error("stack is of only one item!");
// 	if (tail->prev->next != tail || tail->next->prev != tail
// 		|| head->prev->next != head)
// 		own_error("tail data is incorrect!");
// 	if (head->prev->next != head || head->next->prev != head
// 		|| head->prev->next != head)
// 		own_error("head data is incorrect!");
